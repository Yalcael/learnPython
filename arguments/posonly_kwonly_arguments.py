...
assert sys.version_info >= (
    3,
    8,
), "positional-only arguments are a Python 3.8+ feature, upgrade your Python!"
...
def power_mod(x, y, /, *, mod):
    return (
        x**y
    ) % mod  # Note: this is a very slow way to do it, instead use fast pow but reduce mod n each step
...
def mix_and_match_most_general(
    pos_1, pos_2, /, pos_or_kw_1, pos_or_kw_2, *, kw_1, kw_2, **kwargs
):
    pass
...
def speed_differences():
    ...
    t1 = (
        timeit(stmt="func(1, 2, 3)", globals={"func": func}, number=trials)
        / trials
        * display_scale
    )
    t2 = (
        timeit(stmt="func(a=1, b=2, c=3)", globals={"func": func}, number=trials)
        / trials
        * display_scale
    )
    t3 = (
        timeit(stmt="func(c=3, a=1, b=2)", globals={"func": func}, number=trials)
        / trials
        * display_scale
    )
    t4 = (
        timeit(stmt="func(1, c=3, b=2)", globals={"func": func}, number=trials)
        / trials
        * display_scale
    )
    ...
    t7 = (
        timeit(stmt="func(c=3, b=2, a=1)", globals={"func": func}, number=trials)
        / trials
        * display_scale
    )
