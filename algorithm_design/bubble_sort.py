def swap(data, a, b):
    ...
    data[b] = temp  # And then put the temporary one that we store beforehand in the other place
...
data = [6, 5, 4, 3, 2, 1]
for i in range(1, len(data)):   # First loop, we use a range here because A RANGE represents a sequence of numbers. It starts at 1 and stop at len(data).
    for j in range(1, len(data)):  # Second Loop
        if data[j] < data[j - 1]:    # Basically, we're comparing two adjacent items.
            swap(data, j, j - 1)  # If the one behind is smaller, then we swap them, this is the basic logic of bubble sort.
