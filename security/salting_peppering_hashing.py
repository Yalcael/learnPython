...
def update_password_hashed(
    db, user, hash_fn: Callable[[bytes], bytes], password: str
) -> None:
...
def update_password_hashed_salted(
    db, user, hash_fn: Callable[[bytes], bytes], password: str
) -> None:
    salt = gen_salt()
...
def update_password_hashed_salted_peppered(
    db, user, hash_fn: Callable[[bytes], bytes], password: str
) -> None:
    salt = gen_salt()
